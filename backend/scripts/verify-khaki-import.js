/**
 * Verify khaki trouser counts in database.
 * Usage: node scripts/verify-khaki-import.js
 */
...
(async () => {
  ...
  const khaki = await db.query(`
    SELECT COUNT(p.id)::text AS cnt
    FROM products p
    JOIN categories c ON p.category_id = c.id
    JOIN categories parent ON c.parent_id = parent.id
    WHERE parent.name = 'Trousers' AND c.name = 'Khaki' AND p.is_active = true
  `);
  console.log('Khaki total:', khaki.rows[0]?.cnt);
  process.exit(0);
})().catch((e) => {
  console.error(e);
  process.exit(1);
