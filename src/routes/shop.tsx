...
export const Route = createFileRoute("/shop")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Browse the full Prince Esquare menswear collection — suits, shirts, shoes, casual and more." },
    ],
  }),
  component: ShopPage,
});
  ...
  useEffect(() => {
    (async () => {
      ...
      const [{ data: cs }, { data: ps }] = await Promise.all([
        ...
        supabase
          .from("products")
          .select("id,slug,title,price,sale_price,category_id,product_images(image_url),categories(name)")
  ...
  return (
    <div className="container mx-auto px-4 py-12 md:py-16">
      <div className="mb-8 text-center">
        <p className="text-xs font-semibold uppercase tracking-[0.25em] text-gold">The Collection</p>
      ...
      {!loading && filtered.length === 0 && (
        <p className="py-16 text-center text-muted-foreground">
          No products in this category yet. <Link to="/shop" className="text-gold hover:underline">View all</Link>
        </p>
      )}
    </div>
  );
