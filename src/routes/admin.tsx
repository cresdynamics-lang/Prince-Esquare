...
function AdminPage() {
  ...
  const loadAll = async () => {
    if (!isStaff) return;
    const [{ data: os }, { data: ps }, { data: ms }, { data: vs }, { data: pr }] =
      await Promise.all([
        supabase
          .from("orders")
        ...
        supabase
          .from("products")
          .select("id, title, price, is_published, categories(name)")
          .order("created_at", { ascending: false })
          .limit(20),
        supabase
                  ...
                  <tr key={p.id} className="border-t border-border">
                    ...
                    <td className="p-3">
                      <button
                        ...
                        className={`rounded px-2 py-1 text-xs ${p.is_published ? "bg-success/15 text-success" : "bg-muted text-muted-foreground"}`}
                      >
