...
export const Route = createFileRoute("/category/$slug")({
  ...
  notFoundComponent: () => (
    <div className="container mx-auto px-4 py-24 text-center">
      ...
      <Link to="/shop" className="mt-6 inline-block text-gold hover:underline">Browse all →</Link>
    </div>
  ),
});
...
function CategoryPage() {
  ...
  const [cat, setCat] = useState<{ name: string; description: string | null; image_url: string | null } | null>(null);
    ...
    <div>
      <section className="relative overflow-hidden bg-navy text-navy-foreground">
        ...
        <div className="container relative mx-auto px-4 py-16 text-center md:py-24">
          ...
          <h1 className="mt-3 font-display text-4xl font-bold md:text-6xl">{cat?.name ?? "Loading…"}</h1>
          {cat?.description && (
            <p className="mx-auto mt-4 max-w-xl text-sm text-navy-foreground/70 md:text-base">{cat.description}</p>
          )}
        </div>
      </section>
